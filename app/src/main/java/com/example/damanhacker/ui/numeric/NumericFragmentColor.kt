...
import com.example.damanhacker.databinding.NumericFragmentColorBinding
...
class NumericFragmentColor : Fragment(), onResultList {

    private lateinit var binding: NumericFragmentColorBinding
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(
            inflater, R.layout.numeric_fragment_color, container, false
        )
        ...
        when (pattern) {
            0 -> {
                SearialNumberClasic().patternCheckBasedOnSerialNumber(
                    listData, this@NumericFragmentColor, 6
                )
            }
            1 -> {
                CheckNumberBasics().patternCheckBasedOnSerialNumber(
                    listData, this@NumericFragmentColor, 9
                )
            }
            2 -> {
                CheckNumberBasics().patternCheckBasedOnSerialNumber(
            ...
            3 -> {
                CheckViolet().patternCheckBasedOnSerialNumber(
                    listData, this@NumericFragmentColor, 0
                )
            }
            4 -> {
                CheckViolet().patternCheckBasedOnSerialNumber(
                    listData, this@NumericFragmentColor, 5
                )
            }
            5 -> {
            ...
            8 -> {
                CheckVioletThreePattern().patternCheckBasedOnSerialNumber(
                    listData, this@NumericFragmentColor, 1
                )
            }
            9 -> {
                CheckVioletThreePattern().patternCheckBasedOnSerialNumber(
                    listData, this@NumericFragmentColor, 2
                )
            }
            10 -> {
                CheckVioletThreePattern().patternCheckBasedOnSerialNumber(
                    listData, this@NumericFragmentColor, 3
                )
            }
            11 -> {
                CheckVioletThreePattern().patternCheckBasedOnSerialNumber(
                    listData, this@NumericFragmentColor, 4
                )
            }
            12 -> {
                CheckVioletThreePattern().patternCheckBasedOnSerialNumber(
            ...
            14 -> {
                CheckVioletThreePattern().patternCheckBasedOnSerialNumber(
                    listData, this@NumericFragmentColor, 7
                )
            }
            15 -> {
                CheckVioletThreePattern().patternCheckBasedOnSerialNumber(
                    listData, this@NumericFragmentColor, 8
                )
            }
            16 -> {
                CheckVioletThreePattern().patternCheckBasedOnSerialNumber(
    ...
    private fun patternRecyclerView(data: ArrayList<patternData>) {
        ...
        binding.recyclerViewPattern.apply {
            ...
            adapter = AdapterPattern(
                list = data, context = requireContext(), onResultList_ = this@NumericFragmentColor
            )
            addItemDecoration(itemDecoration)
        }
    }
